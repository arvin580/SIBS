...
title = '.'.join(sys.argv[1].split('.')[1:3])

R=r'''

library(DNAcopy)
cn <- read.table("%s",header=T)
CNA.object <-CNA( genomdat = cn[,7], chrom = cn[,1], maploc = cn[,2], data.type = 'logratio', sampleid = "%s"
)
...
'''%(sys.argv[1],title,sys.argv[1],sys.argv[1])
