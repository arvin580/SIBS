...
if(exists($gene_name_hash{">$gene[0]"})&& exists($gene_name_hash{">$gene[1]"}))
{
	...
	for($i=0;$i<$fi;$i++)
	{
		for($j=0;$j<$se;$j++)
		{
			$lf=length($first[$i]);
			$ls=length($second[$j]);
			printf OUTFILE3 ">$gene[0]:$gene[1]:$i:$j:$lf:$ls\n";
			$fusion=$first[$i].$second[$j];
			printf OUTFILE3 "$fusion\n";
                        
			printf OUTFILE3 ">$gene[1]:$gene[0]:$j:$i:$ls:$lf\n";
