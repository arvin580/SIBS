...
for inF in F:
    if inF[-4:]=='.mgf':
        ...
        ouFile.write('\t<note type="input" label="list path, taxonomy information">taxonomy2.xml</note>\n')
        ...
        ouFile.write('\t<note type="input" label="spectrum, path">input2/%s</note>\n'%inF)
        ouFile.write('\t<note type="input" label="output, path">output2/%s</note>\n'%ouF)
        ouFile.write('</bioml>\n')
        ...
        ouF2 = inF[0:-4]+'.sh'
        ouFile2 = open(ouF2,'w')
        cwd = os.getcwd().split('/inputxml2')[0]
        ouFile2.write('cd %s\n\n'%cwd)
        ouFile2.write('tandem.exe inputxml2/%s\n'%ouF)
        ouFile2.close()
        L.append(ouF2)
