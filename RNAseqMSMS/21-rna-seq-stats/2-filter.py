def num(inF):
    ...
    for line in inFile:
        ...
        '''
        if (query_start1 + query_end1) <= (query_start2 + query_end2):
            if 1 <= query_start1 <= 10 and 20 <= query_end1  <= 56 \
                    and 20 <= query_start2 <= 56 and 56 <= query_end2 <= 76:
                ouFile.write(line + '\n')
            else:
                ouFile2.write(line + '\n')
        else:
            if 20 <= query_start1 <= 76 and 66 <= query_end1  <= 76 \
                    and 1 <= query_start2 <= 10 and 20 <= query_end2 <= 56:
                ouFile.write(line + '\n')
            else:
                ouFile2.write(line + '\n')
