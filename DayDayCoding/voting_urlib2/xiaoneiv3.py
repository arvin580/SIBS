...
opt['p']='[{"t":"univ","name":"中国科学院上海生命科学研究院"},{"gend":"女生","t":"base"}]'
...
#                inFile.close()
        ...
        #req=urllib2.Request('http://photo.renren.com/photo/'+usrid+'/album/relatives')
        res=openner.open(req)
        html=res.read()
        parser=BeautifulSoup(html)
        albums=parser.findAll('a',stats="album_album")
        #albums=parser.findAll('a',stats="album_cover")
        for item in albums :
            re5=re.search(r'href="(.*)" stats="album_album"><img',str(item))
            #re5=re.search(r'href="(.*)" stats="album_cover"><img',str(item))
            if re5 :
...
#                        re7=re.search(r'src="(.*.jpg)',str(pics[0]))
#                        if re7 :
#                            pic=re7.group(1)
#                            data =urllib.urlopen(pic).read()
#                            inFile=open(name+str(time.time())+'.jpg','wb')
