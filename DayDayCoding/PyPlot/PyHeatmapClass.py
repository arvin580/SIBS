...
class PyHeatmap():
    ...
    def showDataColor(self, ax, color, fig):
        axm = ax.matshow(self.data, cmap=color, aspect='auto', origin='lower')
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    def showDataNameColor(self, ax, color, fig):
        ...
        ax.set_xticklabels(self.colname)
        ax.set_yticklabels(self.rowname)
        axcolor = fig.add_axes([0.2,0.05,0.7,0.02])
        plt.colorbar(axm, cax=axcolor, orientation='horizontal')
        print(self.rowname)
 
    def heatmap(self, color=plt.cm.jet):
        fig = plt.figure()
        ...
        self.showDataNameColor(ax, color, fig)
        plt.show()
