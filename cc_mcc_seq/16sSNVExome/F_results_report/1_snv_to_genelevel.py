...
def snv_level_to_gene_level(iFile,sampleNum) :
    ...
    for line in inFile :
        line=line.strip()
        fields=line.split('\t')
        dict1.setdefault(fields[1],[0]*sampleNum)
        list1.append(fields[1])
        for i,item in enumerate(fields[-sampleNum:]) :
            dict1[fields[1]][i]+=int(item)
...
snv_level_to_gene_level('SNV.genome.somatic.nonsynonymous.UTR.ncRNA.geneLevel.recurrent.mutation.both',8)
