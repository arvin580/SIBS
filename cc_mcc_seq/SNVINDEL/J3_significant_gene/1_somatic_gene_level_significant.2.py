...
def gene_significant_mutated(iFile,samplePositionList,filename) :
    ...
    inFile.close()
    inFile=open('hg19_refGene_utr_max_len')
    for line in inFile :
        line=line.rstrip()
        fields=line.split('\t')
        dict1[fields[0]]+=int(fields[1])
...
#gene_significant_mutated('genome.somatic.exome.UTR.INDEL.nondbsnp.geneLevel',[-8,-7,-6,-5],'ICC')
gene_significant_mutated('genome.somatic.exome.UTR.INDEL.nondbsnp.geneLevel',[-4,-3,-2,-1],'CHC')
#gene_significant_mutated('genome.somatic.exome.UTR.INDEL.nondbsnp.geneLevel',[-8,-7,-6,-5,-4,-3,-2,-1],'ICC_CHC')
