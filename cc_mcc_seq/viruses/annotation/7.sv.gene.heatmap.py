...
def gene_heatmap(sampleNameList,ouF,figsize=0,rowList=[]):
    LD = []
    geneList = []
    inFile = open('viruse.hg.gene.distribution')
    for line in inFile :
        line = line.strip()
        fields = line.split('\t')
...
gene_heatmap(['ICC4A','ICC4B','ICC5A','ICC5B','ICC9A','ICC9B','ICC10A','ICC10B','CHC5A','CHC5B','CHC6A','CHC6B','CHC7A','CHC7B','CHC10A','CHC10B'],'viruse.hg.gene.distribution.heatmap.pdf')
