dict1=dict()
dict2=dict()
inFile=open('sum_snp.genome_combined.sorted.pass012.new.atcg.coding_noncoding')
for line in inFile :
    line=line.strip('\r\n')
    fields=line.split('\t')
...
p0 = ax.bar(ind, hunMean[type3[0]],   width, color='r', yerr=hunStd[type3[0]])
p1 = ax.bar(ind+width, hunMean[type3[1]],   width, color='y', yerr=hunStd[type3[1]])
p2 = ax.bar(ind+2*width, hunMean[type3[2]],   width, color='b', yerr=hunStd[type3[2]])
p3 = ax.bar(ind+3*width, hunMean[type3[3]],   width, color='g', yerr=hunStd[type3[3]])
...
plt.savefig('sum_snp.genome_combined.sorted.pass012.new.atcg.mcc.coding_noncoding.pdf')
