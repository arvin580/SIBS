...
suffix = '.jmp.br.fasta.blasted.filtered.head1'
svs = []
...
def cmp_func(x,y):
    if chrs.index(x[0]) < chrs.index(y[0]):
        ...
        if chrs.index(x[2]) < chrs.index(y[2]):
            return -1
        elif chrs.index(x[2]) > chrs.index(y[2]):
            return 1
        else:
            if x[1] < y[1]:
                return -1
...
for sp in samples:
    ...
    for line in inFile:
        line = line.strip()
        fields = line.split(':')
        sv = [fields[1],int(fields[2]),fields[3],int(fields[4])]
        svs.append(sv)
