...
metastasis=dict()
inflammation=dict()
inFile5=open('/netshare1/home1/szzhongxin/proj1/hansun/genePathway/geneKnown/fudanexcel/gene_symbol_stemcell_found','r')
for line in inFile5 :
    line=line.strip()
    fields=line.split('\t')
    for item in fields :
        stemcell[item]=''
inFile5.close()
inFile5=open('/netshare1/home1/szzhongxin/proj1/hansun/genePathway/geneKnown/fudanexcel/gene_symbol_metastasis_found','r')
for line in inFile5 :
    line=line.strip()
    fields=line.split('\t')
    for item in fields :
        metastasis[item]=''
inFile5.close()
inFile5=open('/netshare1/home1/szzhongxin/proj1/hansun/genePathway/geneKnown/fudanexcel/gene_symbol_inflammation_found','r')
for line in inFile5 :
    line=line.strip()
    fields=line.split('\t')
    for item in fields :
...
for line in inFile2 :
    ...
    fields=line.split()
    type=[]
    if fields[0] in stemcell :
        type.append('stemcell')
    if fields[0] in metastasis :
        type.append('metastasis')
    if fields[0] in inflammation :
        type.append('inflammation')

    ouFile1.write(';'.join(type)+'\t'+line+'\t'+interval_seq(fields[1],int(fields[2]),500,500,0,fields[4])+'\t'+interval_seq(fields[1],int(fields[2]),500,500,1,fields[4])+'\n')
